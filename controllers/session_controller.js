...
exports.forgotten = function(req, res, next){
 ...
 .success(function(user){
     if(user){
      ...
      user.save(['salt','hashed_password'])
      .success(function(){
...
exports.create = function(req, res, next) {
    ...
    uc.autenticar(req._remoteAddress, login, password, function(error, user) {
        ...
        .success(function(user) {
          ...
          user.save(['ultimaConexion'])
          .success(function(){
          req.session.user = {id:user.id, login:user.login, name:user.name, last_activity:sumMinutes(new Date())};
           models.Grab
        .findAll({ where: { userid: req.session.user.id, estado : 0, leido : 0}})
...
text: 'Hola '+name,
// HTML body
html: '<p><b>Te envío una contraseña provisional para recuperar tu contraseña de SGRP</b></p>'+
      '<br><p>Tu nueva contraseña es: <strong>'+password+'</strong><br></p><p>Por favor, después de entrar, cambiarla por una nueva para recordarla mejor</p>'+
      '<p>Un saludo</p><br><h3>El equipo SGRP</h3>'
};
...
function generateRandomPassword(){
  var c = ['a','A','7','b','B','c','C','d','D','4','e','E','f','F','5','g','G','h','H','i','8','I','j','J','k','9','K','l','L','m','2','M','n','N','o','O','p','P','q','6','Q','r','R',
           's','S','t','3','T','u','U','v','V','w','1','W','x','X','y','Y','z','Z','0'];
   var password = '';        
   var i = 9;
   while(i != 0){
